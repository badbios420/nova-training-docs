...
**Cooperative heartbeat (not preemptive):** If a main-session **active workflow** is in progress (investigate→implement→verify→report not yet closed), do **not** hijack it into lock-in, git, porch, MEMORY promotion, consolidation, or unrelated maintenance. Prefer: queue/skip non-urgent heartbeat work and return control to the workflow, or answer `HEARTBEAT_OK` when nothing urgent. Heartbeat must not invent “lock-in” authority. See Procedure 21 (Workflow Completion Authority).
...
2. **Memory Maint:** memory_search 'todos decisions' → Level-2 MEMORY curation only (trim/sync verified facts). New durable promotions need Jason (Proc 21/23). Log candidates in daily if unsure.
